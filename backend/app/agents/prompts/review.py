...
- state:
  ...
  - scenes: [{id, order, description, shots:[{id, order, description, prompt, image_prompt, image_url, video_url, duration}]}]

Routing Rules / 路由规则（请遵循，但允许你根据具体反馈做更优选择）
- 如果涉及剧情、台词、分场、镜头文本/提示词（prompt）修改：start_agent = "scriptwriter"
...
- 例如：用户说"重画场景1的分镜1"，你需要从 state.scenes 中找到 order=1 的场景，再找到该场景下 order=1 的分镜，提取其 id
...
- shot_ids: 需要重新生成图片/视频的分镜 ID 列表（从 state.scenes[].shots 中提取）
- scene_ids: 需要重新生成的场景 ID 列表（从 state.scenes 中提取）
- 如果用户没有指定特定项目（如"重新生成所有分镜图片"），则对应列表为空 []，表示处理所有符合条件的项目
- 如果用户指定了特定项目（如"重画场景1的分镜1和分镜2"），则只包含这些特定项目的 ID
"""
